...
export default class AppStart extends Command {
  static description = "Start a stopped app";
  static args = { app: Args.string({ description: "App name or ID", required: true }) };
  async run() {
    const { args } = await this.parse(AppStart);
    const appId = await resolve(args.app);
    await api(`/api/v1/${PAAS}/apps/${appId}/`, {
      method: "PATCH", body: { fields: ["is_enabled"], is_enabled: true },
    });
    this.log(`Started "${args.app}".`);
  }
}
